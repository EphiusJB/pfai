...
//   };
//   const createProfile = async (user, url, username) => {
...
//         await createProfile(data.user, url, username);
//       }
...
//               type="button"
//               onClick={() => setStep(0)}
//               className="flex-none h-11 px-4 rounded-xl border text-[13px] font-semibold transition-colors hover:bg-[var(--color-primary)]"
//               style={{
...
export default function SignUpPage() {
  ...
  // Consume store actions & loading state
  ...
  useDesktopAutofocus(firstNameRef, step === 0);
  useDesktopAutofocus(passwordRef, step === 1);
  const [error, setError] = useState("");
      ...
      {step === 1 && (
        <form onSubmit={handleSubmit} className="space-y-4" noValidate>
          ...
          <div className="flex gap-2">
            ...
            <SubmitButton loading={isLoading} className="flex-1">
